...
public class ResultView {
    ...
    private static final String MATCH_MESSAGE_WITH_BONUS =
            "%d개 일치, 보너스 볼 일치 (%d원)- %d개";
    private static final String RATE_OF_RETURN_MESSAGE = "총 수익률은 %f입니다.";
    private static final String RATE_OF_RETURN_ADDITIONAL_MESSAGE =
    ...
    private static void displayWinningList(LottoResult lottoResult) {
        ...
        for (int i = lottoRanks.length - 1; i >= 0; i--) {
            ...
            int winningCount = lottoResult.getWinningCount(lottoRank);
            displayWinning(lottoRank, winningCount);
        }
    }
